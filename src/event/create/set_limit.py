    ...
    if not context.user_data.get("restored"):
        ...
        try:
            ...
            try:
                ...
                message = f"✅ Мероприятие создано!\n\n📢 <a href='{event_link}'>{draft['description']}</a>\n📅 Дата: {draft['date']}\n🕒 Время: {draft['time']}"
                await context.bot.send_message(chat_id=draft["creator_id"], text=message, parse_mode="HTML")
            except Exception as e:
            ...
            error_message = "Неверный формат лимита. Введите положительное число или 0:"
            try:
